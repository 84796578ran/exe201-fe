...
class PostRepository extends BaseRepository {
  ...
  Future<Post?> getPost(String id) async {
    ...
    return Post.fromDb(
      ...
      wishlist: wishlist
          ...
          .toList(),
      images: images.map((i) => PostImage.fromDb(i)).toList(),
    );
  }
  ...
  Future<void> createPost(Post post) async {
    ...
    await db.transaction((txn) async {
      ...
      final postMap = post.toDb();
      // Remove rating if it's null to use the default value
      if (postMap['rating'] == null) {
        postMap.remove('rating');
      }
      await txn.insert('posts', postMap);

      // Insert images
      for (var image in post.images) {
        await txn.insert('post_images', {
          'id': image.id,
          'postId': post.id,
          'url': image.url,
        });
